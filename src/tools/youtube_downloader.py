...
def download_media(
    ...
    try:
        ...
        # 修正：從 'requested_downloads' 獲取最終檔案路徑，以處理音訊轉換等情況
        requested_downloads = video_info.get('requested_downloads')
        if requested_downloads:
            # 當有音訊轉換時，最後一個項目通常是最終的檔案
            original_filepath_str = requested_downloads[-1].get('filepath')
        else:
            # 作為備援，如果 'requested_downloads' 不存在，則退回使用 '_filename'
            original_filepath_str = video_info.get('_filename')

        if not original_filepath_str:
            raise RuntimeError("yt-dlp 未在其 JSON 輸出中提供可用的檔名資訊。")
