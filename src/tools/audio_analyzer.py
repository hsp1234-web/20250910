...
def upload_file_via_rest(file_path: Path, api_key: str) -> dict:
    ...
    # 根據 API 文件和 POC 的發現，直接傳遞 displayName 和 mimeType
    payload = {
    ...
    # 直接從回應中獲取 uploadUri，因為它不在 'file' 鍵下
    response_json = init_res.json()
    upload_uri = response_json.get("uploadUri")
    if not upload_uri:
        # 如果找不到 uploadUri，則記錄詳細的錯誤並引發異常
        log.error(f"在 API 回應中找不到 'uploadUri'。收到的回應: {json.dumps(response_json, indent=2)}")
        raise KeyError("在檔案上傳的初始 API 回應中找不到 'uploadUri'。")
