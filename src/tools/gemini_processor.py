...
# --- 核心 Gemini 處理函式 (V3 - 全面使用 Client API) ---
...
def validate_key(api_key: str) -> bool:
    """
    僅驗證 API 金鑰的有效性。
    :param api_key: 要驗證的 Google API 金鑰。
    :return: 如果金鑰有效則回傳 True，否則回傳 False。
    """
    if not api_key:
        return False
    try:
...
def main():
    ...
    # 移除了 validate_key，因為它現在是一個直接匯入的函式，不再透過命令列呼叫。
    parser.add_argument("--command", type=str, default="process", choices=["process", "list_models"], help="要執行的操作。")
