...
def run_full_analysis_pipeline(task_id: str, item_ids: List[int], db_client: DBClient, task_manager):
    ...
    with httpx.Client(timeout=600.0) as client:
        for item_id in item_ids:
            ...
            try:
                # --- 準備階段 ---
                # (Jules @ 2025-10-11) 核心修正：不再於此處清空歷程。
                # 清空操作應由一個獨立的 "重試" API 觸發，以避免覆蓋正在進行或已失敗的任務歷程。
...
@router.post("/retry_analysis", status_code=200, summary="重置一個失敗的項目以便重試")
async def retry_analysis(
    request: RetryRequest,
    db_client: DBClient = Depends(get_db_client)
):
    """
    (Jules @ 2025-10-11) 新增：重置單一項目的狀態以便重新處理。
    - 清空 processing_history。
    - 重設 status 和 last_error_details。
    """
    log.info(f"收到對項目 ID {request.item_id} 的重試請求。")
    try:
        updates = {
            "processing_history": json.dumps([]),
            "status": "pending",
            "last_error_details": None
        }
        success = db_client.update_url(request.item_id, updates)
        if not success:
            raise HTTPException(status_code=500, detail="更新資料庫時發生錯誤。")

        return {"status": "success", "message": f"項目 {request.item_id} 已成功重置。"}
    except Exception as e:
        log.error(f"重置項目 {request.item_id} 時發生錯誤: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"重置項目時發生伺服器內部錯誤: {str(e)}")
