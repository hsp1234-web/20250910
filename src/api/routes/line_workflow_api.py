...
async def get_latest_workflow(db_client: DBClient = Depends(DBClient)):
    """
...
async def reset_workflow(workflow_id: int, db_client: DBClient = Depends(DBClient)):
    """
...
async def get_workflow_line_item_status(
    workflow_id: int,
    db_client: DBClient = Depends(DBClient)
):
    """
