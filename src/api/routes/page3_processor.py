...
async def reset_files(payload: ResetRequest):
    ...
    try:
        ...
        updated_rows = DB_CLIENT.execute_query(sql, payload.ids)

        if updated_rows == 0:
            log.warning(f"API: 重設檔案狀態時，沒有任何 ID ({payload.ids}) 被更新。")
            raise HTTPException(status_code=404, detail="提供的 ID 在資料庫中不存在或無法被重設。")

        log.info(f"API: 已成功將 {updated_rows} 個檔案的狀態更新為 'completed'。")
        return JSONResponse(content={"message": f"成功重設 {updated_rows} 個檔案。", "reset_ids": payload.ids})
    except Exception as e:
...
async def start_processing(payload: ProcessRequest, background_tasks: BackgroundTasks, request: Request):
    ...
    try:
        ...
        DB_CLIENT.execute_query(sql, url_ids)
