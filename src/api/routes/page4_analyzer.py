...
# --- 重構後的背景任務函式 (同步阻塞部分) ---
# JULES (V7 微服務重構):
# _run_stage1_blocking_task 函式已被移除。其核心邏輯已遷移至 `src/services/analyzer_service.py` 中，
# 並由 Redis 訊息驅動，不再由 HTTP API 直接觸發。
...
# JULES (V7 微服務重構):
# 移除了 /start_stage1_analysis 和 /retry_stage1_analysis API 端點。
# 核心的「下載 -> 處理 -> 分析」流程現在完全由 Redis 訊息佇列自動觸發，
# 不再需要手動從 UI 啟動第一階段分析。
# 其他特定階段的分析（如日期推斷、績效分析、報告生成）的端點被保留，
# 因為它們代表了使用者在 UI 上的特定手動操作。
