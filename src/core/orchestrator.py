...
def install_core_dependencies():
    """安裝核心依賴。"""
    log.info("--- [依賴安裝] ---")
    req_file = ROOT_DIR / "requirements" / "core.txt"
    if not req_file.is_file():
        log.warning(f"找不到核心依賴檔案: {req_file}，跳過安裝。")
        return

    log.info(f"正在從 {req_file} 安裝核心依賴...")
    try:
        command = [sys.executable, "-m", "pip", "install", "--quiet", "-r", str(req_file)]
        run_command(command, log_prefix="pip")
        log.info("✅ 核心依賴安裝完成。")
    except (subprocess.CalledProcessError, subprocess.TimeoutExpired):
        log.critical(f"❌ 核心依賴安裝失敗！協調器無法啟動。")
        raise
