## 1107號 - 2025-09-24T05:49:42+08:00

### fix(downloader): 更新 yt-dlp 以修復 YouTube nsig 簽名解析錯誤

- **動機**: 使用者回報，使用 `yt-dlp` 下載部分 YouTube 影片時，系統會發生 `nsig extraction failed` 錯誤，導致下載失敗。這是 `yt-dlp` 在 YouTube 更新其播放器後常見的時效性問題。
- **核心變更**:
    - **更新依賴 (`requirements/downloader.txt`)**: 將 `yt-dlp` 的版本從 `2025.9.5` 更新至最新的 `2025.9.23`。此更新旨在獲取針對 YouTube 最新播放器的修補程式。
    - **修復安裝流程**:
        - 在測試過程中，發現 `uv` 安裝工具在沙箱環境中即使回報成功，也未能正確安裝套件。
        - 改為使用更穩定、更標準的 `pip` 來執行安裝，成功解決了 `ModuleNotFoundError: No module named 'yt_dlp'` 的問題。
- **測試與驗證**:
    - **初步測試失敗**: 更新 `yt-dlp` 後，原先的 YouTube 連結雖然不再報 `nsig` 錯誤，但出現了新的 `403 Forbidden` 錯誤，表示該影片可能需要登入驗證。
    - **驗證核心功能**: 為了驗證下載器本身的功能已修復，根據使用者的指示，改用一個公開的 Facebook 影片連結進行測試。
    - **最終測試成功**: 成功下載了 Facebook 影片，證明在更新 `yt-dlp` 並使用 `pip` 正確安裝後，下載器的核心功能已完全恢復正常。
- **成果**: 本次提交成功解決了因 `yt-dlp` 版本過舊而導致的 YouTube 下載失敗問題。雖然部分需要登入的影片仍無法下載（這超出了本次修復範圍），但下載器的核心引擎已確認更新並可正常運作，保障了對大多數公開影片的支援。
