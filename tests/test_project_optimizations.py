...
def test_real_user_data_title_is_saved(db_conn):
    """
    使用使用者在 2025-09-17 提供的真實資料，驗證標題能被正確解析並儲存。
    這個測試在修正程式碼後應該要能通過。
    """
    # 步驟 1: 準備使用者提供的真實輸入文字
    chat_text = "2025/6/5（週四）\n18:34\t640-0760313游子龍\t六月小作文-交易者超級心流訓練-心得分享 https://kindhearted-jersey-ac1.notion.site/208f7d7dbdc28094b2f4ce9cb2c54765"
    expected_title = "六月小作文-交易者超級心流訓練-心得分享"
    expected_url = "https://kindhearted-jersey-ac1.notion.site/208f7d7dbdc28094b2f4ce9cb2c54765"
    ...
    # 步驟 3: 從資料庫取回資料並驗證
    cursor = db_conn.cursor()
    cursor.execute("SELECT title FROM extracted_urls WHERE url = ?", (expected_url,))
    saved_item = cursor.fetchone()

    assert saved_item is not None, "資料應該已被存入資料庫"
    assert saved_item['title'] == expected_title, f"資料庫中的標題為 '{saved_item['title']}'，與預期的 '{expected_title}' 不符"
