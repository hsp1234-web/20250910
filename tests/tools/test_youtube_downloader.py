...
# 模擬 yt-dlp 對於 Shorts 影片的 JSON 輸出
# 特點是 'filepath' 不在頂層，而在 'requested_downloads' 陣列中
...
MOCK_YT_DLP_REGULAR_OUTPUT = {
    ...
    "formats": [],
    "thumbnails": [],
    "filepath": "downloads/audio/[m4a]Regular Video Title.m4a",
    "_filename": "downloads/audio/[m4a]Regular Video Title.webm",
}
...
def test_download_media_regular_video_success(temp_audio_dir, mocker):
    ...
    assert output_json["status"] == "已完成"
    assert output_json["output_path"] == expected_path_str
    assert output_json["video_title"] == "Regular Video Title"
