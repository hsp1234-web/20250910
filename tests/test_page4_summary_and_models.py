...
# 延遲匯入 app，以確保路徑已設定
from api.api_server import app, get_notification_queue, get_analysis_semaphore
from api.dependencies import get_db

@pytest.fixture
def client_with_overrides():
    """提供一個帶有模擬依賴的 TestClient。"""

    app.dependency_overrides[get_db] = lambda: MagicMock(spec=DBClient)
    app.dependency_overrides[get_notification_queue] = lambda: MagicMock(spec=asyncio.Queue)
    app.dependency_overrides[get_analysis_semaphore] = lambda: MagicMock(spec=asyncio.Semaphore)
...
client = TestClient(app) # 保留一個原始的 client
...
def test_get_files_for_summary_page_success(client_with_overrides):
    """測試成功獲取待摘要檔案列表。"""
    ...
    response = client_with_overrides.get("/api/analyzer/files_for_summary")
...
@pytest.mark.timeout(130)
def test_get_files_for_summary_page_no_files(client_with_overrides):
    """測試沒有可供摘要的檔案時的情況。"""
    mock_db = app.dependency_overrides[get_db]()
...
@patch('api.routes.page6_keys.GeminiManager')
def test_get_available_models_success(mock_gemini_manager, mock_key_manager):
    """測試成功獲取模型列表。"""
    # 模擬有效的金鑰和模型列表
...
@pytest.mark.timeout(130)
@patch('api.routes.page6_keys.key_manager')
def test_get_available_models_no_valid_keys(mock_key_manager):
    """測試沒有有效金鑰時，無法獲取模型列表。"""
...
def test_start_summary_generation_success(mock_run_wrapper, client_with_overrides):
    """測試成功啟動重點摘要生成任務。"""
    mock_db = app.dependency_overrides[get_db]()
...
@pytest.mark.timeout(130)
def test_start_summary_generation_no_ids():
    """測試請求中沒有提供任務 ID 的情況。"""
