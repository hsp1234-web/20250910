## 1013號 - 2025-09-11T20:25:08.541994+08:00

### feat(analyzer): 實作同步的 AI 分析概念驗證 (PoC)

- **動機**: 根據使用者的指示，需要將一個已成功驗證的、從財經報告中提取結構化資訊的 PoC 腳本，整合進現有的 Web 應用程式中，作為未來完整交易報告系統的第一步。
- **核心變更**:
    - **前端改造 (`page4_analyzer.html`)**:
        - **新增結果顯示**: 在「AI 分析」頁面新增了一個 `<pre>` 區塊，專門用來顯示從後端回傳的 AI 分析結果 (JSON 格式)。
        - **強化 JS 邏輯**: 更新了頁面的 JavaScript，使其在點擊「開始分析」後，能正確呼叫新的後端 API，並將回傳的 JSON 美化後顯示在新的結果區塊中。
        - **修正 API 路徑**: 修正了 JavaScript 中獲取「已處理檔案」和「提示詞列表」的 API 路徑，使其與現有後端路由保持一致。
    - **後端新增同步 API (`page4_analyzer.py`)**:
        - **建立新端點**: 新增了一個**同步**的 API 端點 `@router.post("/run_analysis")`，專為此 PoC 設計，以便立即回傳分析結果。
        - **整合核心模組**: 此端點的邏輯會依序呼叫 `KeyManager` (獲取有效的 API 金鑰)、`PromptManager` (獲取指定的提示詞模板) 和資料庫 (獲取文件文字內容)。
        - **穩健的 API 呼叫**: 借鑒 `gemini_processor.py` 的設計，實作了一個帶有超時控制的輔助函式 `generate_content_with_timeout`，以穩健地呼叫 Gemini API。
        - **完整的錯誤處理**: 為整個流程添加了詳細的錯誤處理，包括金鑰無效、提示詞找不到、資料庫錯誤、AI 回應格式錯誤等情況。
- **成果**: 成功地將一個獨立的 PoC 腳本，轉化為一個整合在 Web 應用程式中的完整功能。使用者現在可以在前端頁面選擇檔案和提示詞，觸發後端進行同步的 AI 分析，並立即在前端看到結構化的 JSON 分析結果，為後續的自動化報告系統奠定了堅實的基礎。
