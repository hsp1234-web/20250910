## 1035號 - 2025-09-13T17:55:15.773554+08:00

### fix(core): 修正 AI 分析任務因未處理例外而崩潰的問題

- **動機**: 使用者回報，當 Gemini API 金鑰用盡或回傳錯誤時，背景分析任務會因兩個未被正確處理的例外而崩潰。第一個是 `ValueError`，因為 `gemini_manager.py` 在金鑰失效時回傳 `None`，而呼叫端預期收到一個三元組。第二個是 `TypeError`，因為 `page4_analyzer.py` 試圖 `raise` 一個非標準的例外物件。
- **核心變更**:
    - **修正 `gemini_manager.py` 的回傳值**:
        - 修改了 `prompt_for_json` 和 `prompt_for_text` 函式，使其在任何情況下都回傳完整的 `(結果, 錯誤, 金鑰名稱)` 三元組，即使在 API 呼叫失敗時也是如此。這確保了呼叫端的解包操作不會失敗，從而解決了 `ValueError`。
    - **改善 `page4_analyzer.py` 的錯誤處理**:
        - 重寫了 `_run_stage1_blocking_task` 和 `_run_stage2_blocking_task` 中的錯誤處理邏輯。
        - 新邏輯會檢查從 `gemini_manager` 收到的 `error` 物件。如果該物件存在，程式現在會安全地記錄詳細錯誤、更新資料庫中的任務狀態為「失敗」，然後正常地從函式返回，而**不是** `raise` 該錯誤。
        - 此項修改徹底避免了 `TypeError`，並將 API 錯誤轉化為一個可追蹤的、對使用者可見的失敗狀態，而不是讓整個背景任務崩潰。
- **測試與驗證**:
    - 程式碼審查 (`request_code_review`) 確認了此修復的正確性與高品質。
    - 透過靜態分析，確認了新的程式碼邏輯能夠覆蓋使用者描述的所有錯誤場景，從根本上解決了問題。
- **成果**: 本次修復極大地提升了 AI 分析流程的穩定性。系統現在能夠優雅地處理 API 金鑰失效或配額用盡等錯誤，將其記錄為任務失敗，而不再導致服務崩潰，確保了其他任務可以繼續執行。
