## 1014號 - 2025-09-11T18:01:07.714233+08:00

### fix(downloader): 修正命名邏輯並增強前端介面

- **動機**: 根據使用者回饋，先前的修改未能完全符合預期。本次更新旨在徹底修正檔案命名邏輯，並根據最新的要求來增強「批次下載」頁面的前端介面。
- **核心變更**:
    - **後端命名邏輯修正**:
        - **還原邏輯**: `src/tools/drive_downloader.py` 和 `src/api/routes/page2_downloader.py` 中的程式碼已被還原。檔案命名時間戳現在**統一並正確地**使用 URL 被加入系統時的 `created_at` 時間，解決了先前的誤解。
    - **前端介面增強**:
        - **增強 API**: `page2_downloader.py` 中的 `/pending_urls` 端點現在會額外查詢並提供 `message_date` 和 `message_time` 欄位，以供前端使用。
        - **豐富化顯示**: `page2_downloader.html` 中的 JavaScript 已被修改。現在，「項目詳情」一欄會顯示一個包含「編號 (LINE時間) 網址」的複合字串，例如 `123 (2025-09-11 10:30) https://...`，提供了更豐富的上下文資訊。
- **成果**: 此次修正確保了檔案命名規則完全符合使用者的最終要求。同時，顯著增強了「批次下載」頁面的資訊密度和易用性，讓使用者能更直觀地辨識待處理的項目。

---
## 1013號 - 2025-09-11T17:35:37.060857+08:00

### feat(downloader): 升級下載頁面 UI 並實作條件式命名

- **動機**: 使用者希望能改善「批次下載」頁面的使用體驗，使其與「網址提取」頁面風格一致。同時，希望能讓檔案命名規則更智慧，優先使用文章的發布時間。
- **核心變更**:
    - **前端 UI 改造 (`page2_downloader.html`, `page2.css`)**:
        - **表格化介面**: 將原本的 `<ul>` 列表，全面重構為與第一頁風格一致的 `<table>` 結構。新的表格包含了勾選框、ID、可點擊的 URL 和加入時間，大幅提升了資訊的清晰度。
        - **樣式統一**: 在 `page2.css` 中移除了舊的列表樣式，並從 `page1.css` 引入了統一的表格樣式，確保了應用程式整體的視覺一致性。
        - **功能對齊**: 「已完成下載」列表也同步升級為表格樣式。
    - **後端條件式命名**:
        - **API 層 (`page2_downloader.py`)**: 修改了 `run_download_task` 函式，使其在從資料庫查詢待下載項目時，一併獲取 `message_date` 和 `message_time`。
        - **核心邏輯 (`drive_downloader.py`)**:
            - 全面重構了 `download_file` 函式，使其接收 `message_date` 和 `message_time`。
            - 實作了**條件式時間戳**邏輯：如果 `message_date` 和 `message_time` 存在，則優先使用此「小作文時間」；如果不存在，則自動使用「下載當下」的時間作為備用方案。
- **成果**: 本次更新同時完成了後端功能的增強與前端體驗的優化。檔案命名變得更加智慧與準確，同時「批次下載」頁面的操作體驗也達到了與其他頁面統一的高標準，讓流程更加順暢。

---
## 1012號 - 2025-09-11T17:12:43.298972+08:00

### refactor(downloader): 調整檔案命名規則以符合使用者要求

- **動機**: 使用者指示，目前的檔案命名格式 `時間戳_file_編號` 不易閱讀且不符合團隊規範。需要將其調整為 `編號_時間戳` 的格式，以提升可追溯性與一致性。
- **核心變更**:
    - **`src/tools/drive_downloader.py`**:
        - **修改命名邏輯**: 在 `download_file` 函式中，將建構檔名的程式碼從 `f"{timestamp}_file_{url_id}{extension}"` 修改為 `f"{url_id}_{timestamp}{extension}"`。
        - **更新註解**: 同時更新了相關的程式碼註解，清楚說明了新的命名格式為 `編號_時間戳.副檔名`，並提供了一個範例。
- **成果**: 檔案命名規則已成功更新。所有透過系統下載的新檔案，現在都會採用 `123_2025-09-11T17-12-43.docx` 這樣更清晰、更符合規範的格式，方便了後續的管理與查找。
