## 992號 - 2025-09-10T20:35:44.754564+08:00

### fix(api): 修正檔案處理API的路由錯誤

- **動機**: 使用者回報，在「頁面三：檔案處理」點擊「開始處理選中項目」按鈕時，會出現 404 Not Found 錯誤，導致功能完全無法使用。
- **核心變更**:
    - **錯誤分析**: 經查閱後端日誌和原始碼，發現問題根源在於 `src/api/routes/page3_processor.py` 中的 API 路由定義錯誤。端點的裝飾器被錯誤地寫為 `@router.post("/api/start_processing")`，其中包含了多餘的 `/api` 前綴。
    - **路由修正**: 將錯誤的裝飾器修正為 `@router.post("/start_processing")`。這使得該路由能與 `api_server.py` 中定義的 `/api/processor` 前綴正確組合，形成正確的、可訪問的 API 路徑 `/api/processor/start_processing`。
- **成果**: 此次修正恢復了「頁面三」的檔案處理功能，使用者現在可以成功提交處理請求，解決了一個關鍵的功能性錯誤。
