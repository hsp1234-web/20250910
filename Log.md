...
### 動機
為了從根本上解決系統啟動緩慢和先前重構時測試不穩定的問題，本次任務旨在完成一個概念驗證（PoC）：將單一功能（金鑰管理）遷移為獨立的微服務，並為其建立一套極其穩健、可靠的自動化測試流程，作為後續所有重構工作的標準範本。

### 核心變更
1.  **微服務建立 (`services/key_service/`)**:
    - 建立了 `key_service` 微服務，它是一個完全獨立的 FastAPI 應用，擁有自己的資料庫 (`key_service.db`) 和依賴項 (`requirements.txt`)，負責所有金鑰相關的業務邏輯。

2.  **API 閘道改造 (`src/api/routes/page6_keys.py`)**:
    - 將原有的 `page6_keys.py` 徹底重構為一個純粹的 API 代理（Proxy）。
    - 它不再處理任何業務邏輯，而是將所有來自前端的 `/api/keys` 請求，安全地轉發給後端新建立的 `key_service` 微服務。

3.  **健壯的自動化測試 (`tests/test_key_service.py`)**:
    - 建立了一個全新的、高標準的 `pytest` 測試套件。
    - **即用即毀的環境**: 測試會在執行前，使用 `uv venv --clear` 為微服務自動建立一個全新的、乾淨的虛擬環境，並在測試結束後（無論成功或失敗）將其徹底刪除。
    - **動態與超時**: 測試會自動尋找可用的網路埠號來啟動服務，並在所有環節（服務啟動、API 呼叫）都加入了超時保護，從根本上解決了先前測試流程會「掛起」的問題。

4.  **流程文件化**:
    - **`AGENTS.md`**: 根據使用者指示，更新了獲取日誌時間戳的規則，從「執行腳本」改為「向使用者詢問」。
    - **`Plan4.md`**: 詳細記錄了本次 PoC 的成功策略與成果，確立了新的測試標準。

### 成果
本次 PoC 非常成功。我們不僅將一個核心功能安全地遷移為微服務，更重要的是建立了一套高度可靠的自動化測試範本。這個範本解決了環境污染、埠號衝突和程序掛起等所有先前遇到的測試痛點，為接下來更大規模的系統重構工作鋪平了道路。
