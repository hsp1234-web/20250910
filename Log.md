## 1105號 - 將績效分析圖表從 Plotly 遷移至 Matplotlib (2025-09-15T16:19:29.630315+08:00)

### 動機
原有的「策略權益曲線」圖表由 Plotly.js 生成，這需要在前端載入一個相對龐大的 JavaScript 函式庫。為了優化載入速度並減少前端依賴，本次更新旨在將圖表生成邏輯完全移至後端，使用 Matplotlib 函式庫在伺服器端直接生成圖表圖片。

### 核心變更
1.  **後端圖表生成邏輯 (`src/tools/quantitative_analyzer.py`)**:
    - **替換函式庫**: `_generate_performance_chart_html` 函式已完全重寫。原先使用 `plotly` 的邏輯被移除，改為使用 `matplotlib`。
    - **伺服器端渲染**: 新的邏輯會在後端記憶體中直接繪製策略權益曲線圖，並將其儲存為 PNG 圖片格式。
    - **Base64 編碼**: 產生的 PNG 圖片會被編碼為 Base64 字串，並嵌入一個 `<img>` HTML 標籤中。這個 HTML 片段最終被回傳給前端。
    - **中文字體處理**: 為了確保圖表中的中文標題與圖例（如「策略權益曲線」）能正確顯示，實作了在執行時從網路下載並載入「思源黑體 (Noto Sans TC)」字型的邏輯，並更新了 URL 以確保穩定下載。

2.  **依賴項管理 (`requirements/analysis.txt`)**:
    - **移除 Plotly**: `plotly` 已從依賴項列表中移除。
    - **新增 Matplotlib**: `matplotlib` 被新增為新的必要依賴。

### 成果
本次更新成功地將圖表生成的負擔從前端轉移到了後端。前端現在只需渲染一個簡單的 `<img>` 標籤，不再需要載入 Plotly.js，從而顯著提升了頁面的載入效能與輕量化程度。同時，透過在後端處理中文字體，確保了圖表的顯示效果與原先一致。
