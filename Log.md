## 993號 - 2025-09-10T19:38:43.659289+08:00

### refactor(ui): 重構為可收合的側邊欄導覽選單

- **動機**: 先前在各頁面頂部的靜態導覽列佔用過多螢幕空間，且設計風格較為粗糙。為了提升介面的現代感與空間利用率，使用者要求將其重構為一個可在所有功能頁面中統一使用的、可收合的側邊欄選單。
- **核心變更**:
    - **重構選單元件 (`menu.html`, `menu.css`)**:
        - **結構**: 完全廢棄了舊的 `header` 結構，改為以 `aside` 標籤實現的側邊欄。新增了一個漢堡圖示按鈕來控制其開合，以及一個在選單展開時覆蓋主內容的半透明遮罩層。
        - **互動**: 在 `menu.html` 中內建了 JavaScript 邏輯，透過監聽點擊事件並切換 CSS class (`.sidebar-open`) 來實現側邊欄的顯示與隱藏，確保了互動功能與元件本身的封裝。
        - **視覺**: `menu.css` 被完全重寫，為新的側邊欄、漢堡按鈕、遮罩層及內部連結提供了現代化的樣式，並加入了流暢的滑入/滑出過渡動畫。返回首頁的連結也被一個更直觀的 SVG 圖示取代。
    - **統一五個功能頁面 (`page1` 到 `page5`)**:
        - **標準化**: 逐一修改了所有五個功能頁面的 HTML 檔案，移除了它們各自舊的、寫死的 `header`。
        - **動態注入**: 在每個頁面中，都加入了一段標準化的 JavaScript 腳本。此腳本會在頁面載入時，非同步地獲取 `menu.html` 的內容，並將其注入到頁面頂部的 `#menu-container` 容器中。
        - **狀態高亮**: 注入腳本同時會根據當前頁面的路徑，為側邊欄中對應的連結加上 `.active` class，以高亮顯示，提供明確的視覺導引。
- **成果**: 成功地將整個應用的導覽系統重構為一個現代化、可響應、空間效率更高的可收合式側邊欄。所有相關頁面的使用者體驗和視覺一致性都得到了顯著提升。
