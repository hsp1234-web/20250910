## 998號 - 2025-09-10T22:17:50.740150+08:00

### fix(core): 修復背景任務通知與 URL 提取的兩項關鍵錯誤

- **動機**: 使用者回報了兩個獨立的問題：(1) 背景任務（如下載、檔案處理）完成後，向上游 API 發送通知時因目標埠號錯誤而導致「Connection refused」失敗。(2) 從文字中提取網址時，若多個網址串接在一起，會被錯誤地解析為單一的損毀網址。
- **核心變更**:
    - **修復通知埠號錯誤**:
        - **問題分析**: 先前的實作 (`request.url.port`) 在特定部署環境（如反向代理後）下無法取得正確的後端伺服器埠號，導致背景任務向錯誤的埠號 (例如 8007) 發送通知。
        - **程式碼修復 (`api_server.py`, `page2_downloader.py`, `page3_processor.py`)**:
            - 在 `api_server.py` 中新增了一個 FastAPI 中介軟體。此中介軟體會在應用程式啟動時，從 ASGI scope 中可靠地捕獲伺服器實際監聽的埠號，並將其儲存於全域應用狀態 `app.state.server_port`。
            - 修改了 `page2_downloader.py` 和 `page3_processor.py` 中的路由，使其不再依賴不可靠的 `request.url.port`，而是從 `request.app.state.server_port` 讀取正確的埠號，再傳遞給背景任務。
    - **修復 URL 提取邏輯**:
        - **問題分析**: `url_extractor.py` 的正規表示式在處理無空格分隔的連續網址 (例如 `...end_of_wordhttps://...`) 時，會將其貪婪地匹配為一個單一的、格式錯誤的網址。
        - **程式碼修復 (`src/tools/url_extractor.py`)**:
            - 在 `extract_urls` 函式中，新增了一個基於正規表示式的預處理步驟 (`re.sub(r'(https?://)', r' \1', text)`)。
            - 此變更會聰明地在所有 `http://` 或 `https://` 協議頭前插入一個空格，從而有效地將串接的網址分離，確保後續的正規表示式能正確地將其識別為獨立的個體。
- **成果**:
    - **通知系統的穩定性**: 徹底解決了「Connection refused」錯誤，確保背景任務現在能可靠地將完成狀態通知回主應用程式，從而觸發即時的前端更新。
    - **URL 提取的準確性**: 大幅提升了網址提取的準確性，避免了因格式錯誤而導致的後續下載失敗問題。
