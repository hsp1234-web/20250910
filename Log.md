## 998號 - 2025-09-10T21:22:20.730520+08:00

### fix(core): 修復背景任務通知與 URL 提取的錯誤

- **動機**: 使用者回報，背景下載任務完成後，通知 API 呼叫失敗，導致前端無法更新。同時，也發現 URL 提取邏輯在處理某些貼上文字時會產生損壞的網址。
- **核心變更**:
    - **修復背景任務通知失敗 (Connection Refused)**:
        - **問題分析**: 先前試圖從 `request` 物件直接傳遞埠號給背景任務的方式不可靠。
        - **`api_server.py`**: 實作了一個新的中介軟體 `PortCaptureMiddleware`，它會在伺服器收到第一個請求時，安全地捕獲動態分配的埠號並儲存於 `app.state.port`。
        - **`page2_downloader.py` & `page3_processor.py`**: 修改了啟動背景任務的 API 端點，使其從 `request.app.state.port` 讀取埠號，確保背景任務能使用正確的埠號來呼叫內部通知 API。
    - **修復 URL 提取邏輯**:
        - **`url_extractor.py`**: 增強了 `extract_urls` 函式。現在它會先對輸入文字進行預處理，在每個 `http` 協議頭前插入一個空格，這能有效分離因複製貼上而串接在一起的 URL。同時，也增加了移除 URL 尾隨標點符號的清理步驟。
- **成果**: 此次提交一次性解決了兩個獨立的問題。背景任務通知機制現在變得更加穩健可靠，徹底解決了「Connection Refused」錯誤。同時，URL 提取的準確性也得到提升，減少了因資料來源不規範而導致的後續下載失敗問題。
