## 1003號 - 2025-09-11T02:08:07.805669+08:00

### fix(downloader): 徹底解決檔名與時間戳不一致問題

- **動機**: 追蹤並修復日誌 #1002 中未完全解決的問題。先前的修復邏輯在 `gdown` 無法從網路回應中偵測檔名時，會產生沒有副檔名的無效檔案 (例如 `..._downloads`)，導致後續的檔案處理程序因 `FileNotFoundError` 而失敗。此外，檔名中的時間戳（下載完成時間）與使用者介面顯示的時間戳（URL 建立時間）不一致，造成混淆。
- **核心變更**:
    - **強化下載器 (`src/tools/drive_downloader.py`)**:
        - **強制偵測副檔名**: 引入 `python-magic` 函式庫。現在，無論 `gdown` 的結果如何，程式都會讀取下載檔案的內容，透過其「魔術數字」來準確識別檔案的真實 MIME 類型，並對應到正確的副檔名 (如 `.pdf`, `.docx`)。這確保了檔案永遠擁有正確的副檔名。
        - **統一檔案基礎名稱**: 放棄使用不穩定的網路原始檔名，改為使用資料庫中的 `url_id` 來產生穩定的基礎名稱 (格式為 `file_{id}`）。
        - **統一時間戳**: 修改了下載流程，將 URL 的「建立時間」(`created_at`) 從 API 層一路傳遞到下載工具。下載工具現在使用這個唯一的初始時間戳來命名檔案。
    - **API 修改 (`src/api/routes/page2_downloader.py`)**:
        - 修改了 `run_download_task` 背景任務，使其從資料庫中讀取 `created_at`，並將其與 `url_id` 一同傳遞給新的下載器函式。
    - **擴充測試 (`tests/test_tools.py`)**:
        - 新增了 `test_drive_downloader_new_naming_logic` 測試案例。此測試透過模擬 `gdown` 和 `python-magic` 的行為，嚴格驗證了新的命名邏輯，確保檔名能根據傳入的 `url_id` 和 `created_at` 正確生成，並附帶正確的副檔名。
- **成果**: 此次更新從根本上解決了檔名生成的所有已知問題。
    - **檔名格式統一**: 最終檔名格式固定為 `YYYY-MM-DDTHH-MM-SS_file_{id}.ext` (例如 `2025-09-10T17-30-08_file_42.pdf`)。
    - **時間戳一致**: 檔名中的時間戳現在與資料庫中的建立時間完全一致，消除了介面顯示的困惑。
    - **處理流程穩定**: 由於檔案現在總是有正確的副檔名，後續的檔案處理流程得以穩定運行，徹底杜絕了先前因檔名問題導致的 `FileNotFoundError`。
...
- **核心變更**:
    - **智慧化下載器 (`src/tools/drive_downloader.py`)**:
        ...
        - **自動偵測檔名**: 新邏輯會讓 `gdown` 函式庫自動從網路回應中偵測原始檔名和副檔名。
